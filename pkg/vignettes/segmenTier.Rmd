...
`csim`, are provided by `segmenTier`'s clustering function
`clusterTimeseries` using Pearson correlations to cluster centroids,
but can also be provided by the user, as outlined below.
...
by `E` and choosing a `nui`>1 on scoring function values.

The effects of these parameters on segmentation are explored in
@Machne2017 and demonstrated in the R demo "segment_data".
...
the setup for segmentation (calculation of `csim`), and
