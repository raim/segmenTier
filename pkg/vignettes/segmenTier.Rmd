...
similarity of a segment from position $j$ to $k$ to cluster
...
matrix (see below), and scoring function "icor" requires only a
position-cluster similarity matrix.  These matrices, internally called
`csim`, are provided by `segmenTier`'s clustering function using
Pearson correlations to cluster centroids, but can also be provided by
the user, as outlined below.
...
similarity. Argument `nui` of `segmentClusters` will be the
self-similarity of the nuisance segment, and `-nui` the nuisance
similarity to all other clusters. Note that the exponent `E` will also
be applied to nuisance similarity `nui`.

The figure below shows the effects of scaling Pearson correlations
by `E` and choosing a `nui`>1.

The effects of these parameters on segmentation are intensively
discussed in the @Machne2017 and demonstrated in the R demo
`segment_data`.
...
arrows(x0=-.3, x1=.3, y0=.25,code=3,lwd=2,lty=2, length=0.1, col="gray")
text(0,.5, expression(score %->%  0), col="gray")
```

## User-Defined Similarities

`segmenTier`'s clustering interface pre-calculates the cluster-cluster
("ccor") and position-cluster ("icor") similarity matrices and adds
them to the "clustering" object that is passed to the recursion
...
labeling must be integers that serve as column and row indices of this
matrix, ie. similarity between a cluster "2" and a cluster "3" is
stored in `csim[2,3]` for scoring function "ccor", using cluster-cluster
similarities and a vector of cluster labels. 
...
ie. non-zero integers, EXCEPT for positions pre-determined as
"nuisance" with fixed similarities `nui`. At such position the cluster
label should be "0".

Consequently, the reported cluster labels of segments are the column
indices of `csim`.

Scaling `E` and nuisance similarities `nui` can be used, but when 
choosing `E`=1 and simply NOT supplying any clusters labels "0" they
will have no effect.

In summary, users that wish to define their own similarities are best
advised to provide this similarities as a numeric matrix, argument
`csim` in the main function `segmentClusters`.  Construction of `csim`
